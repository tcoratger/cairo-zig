...
pub fn benchmark(comptime B: type) !void {
        ...
        inline for (functions) |f| {
            ...
            while (i < args.len) : (i += 1) {
                ...
                res = if (i < arg_names.len)
                    try printBenchmark(writer, res, f.name, formatter("{s}", arg_names[i]), max, max, max, max, max)
                else
                    try printBenchmark(writer, res, f.name, i, max, max, max, max, max);
            }
        }
        break :blk res;
    };
    ...
    inline for (functions) |def| {
        inline for (args, 0..) |arg, index| {
            ...
            const runtime_mean: f64 = @as(f64, @floatFromInt(timer.read())) / @as(f64, @floatFromInt(max_iterations));
            // var d_sq_sum: u128 = 0;
            ...
            if (index < arg_names.len) {
                ...
                _ = try printBenchmark(stderr, min_width, def.name, arg_name, max_iterations, min, max, variance, runtime_mean);
            } else {
                _ = try printBenchmark(stderr, min_width, def.name, index, max_iterations, min, max, variance, runtime_mean);
            }
            try stderr.writeAll("\n");
            try stderr.context.flush();
        }
